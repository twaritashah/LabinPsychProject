...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\adosh\\Desktop\\Lab_Psychology\\Project\\LDTProject.py',
...
trials = data.TrialHandler(nReps=5.0, method='random', 
    ...
    trialList=data.importConditions('C:/Users/adosh/Downloads/conditions.xlsx'),
...
for thisTrial in trials:
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    Distractor.setText(word_invalid)
    # keep track of which components have finished
    trialComponents = [fixation, Target, key_resp, Distractor]
    ...
    while continueRoutine:
        ...
        if Target.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Target.tStartRefresh + 6.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if Distractor.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Distractor.tStartRefresh + 6.0-frameTolerance:
                # keep track of stop time/frame for later
